...
export function BrandingPanel({ draft, edits, onImageRejected }: PanelProps) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <TextField label={t('inspector.branding.brandName', 'Brand name')} value={body.logoText} onFocus={edits.beginEdit} onChange={(value) => edits.set('logoText', value)} className="font-bold" testId="invoices-brand-name" />
      <div>
        <PanelLabel id="invoices-logo-mark-label" className="mb-2">
          {t('inspector.branding.logoMark', 'Logo mark')}
        </PanelLabel>
      ...
      <div>
        <PanelLabel className="mb-2">{t('inspector.branding.logoImage', 'Logo image')}</PanelLabel>
        <div className="flex gap-[6px]">
          ...
          >
            ...
            {t('inspector.branding.uploadLogo', 'Upload logo')}
          </FileLabel>
          {body.logoImage === '' ? null : (
            <button
              type="button"
            ...
            >
              {t('inspector.branding.removeLogo', 'Remove')}
            </button>
          )}
        </div>
        <Note className="mt-[7px]">
          {t('inspector.branding.logoNoteBefore', 'An uploaded logo replaces the mark above. All fixed images live under')} <b>{t('inspector.branding.logoNoteBold', 'Images')}</b>{' '}
          {t('inspector.branding.logoNoteAfter', 'in the toolbar.')}
        </Note>
      </div>
      <Hint icon={Palette}>
        {t('inspector.branding.accentHintBefore', 'Change the accent colour under the')} <b>{t('inspector.branding.accentHintBold', 'title')}</b>{' '}
        {t('inspector.branding.accentHintAfter', 'section.')}
      </Hint>
    </div>
  );
}
