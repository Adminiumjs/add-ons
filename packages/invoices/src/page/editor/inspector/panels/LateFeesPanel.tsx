...
export function LateFeesPanel({ draft, edits, onSelect }: PanelProps) {
  ...
  return (
    <div className="flex flex-col gap-[14px]">
      <SuffixField label={t('inspector.latefees.rate', 'Late fee rate')} suffix={t('inspector.latefees.rateUnit', '% per month')} value={body.lateRate} onFocus={edits.beginEdit} onChange={(value) => edits.set('lateRate', value)} testId="invoices-late-rate" />
      <SuffixField
        label={t('inspector.latefees.grace', 'Grace period')}
        suffix={t('inspector.latefees.graceUnit', 'days')}
