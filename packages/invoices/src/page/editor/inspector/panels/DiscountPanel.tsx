...
export function DiscountPanel({ draft, edits, onSelect }: PanelProps) {
  return (
    <div className="flex flex-col gap-[9px]">
      <PanelLabel className="mb-0">{t('inspector.discount.codes', 'Discount codes')}</PanelLabel>
      <RowsEditor
        rows={draft.body.discCodes}
        addLabel={t('inspector.discount.add', 'Add code')}
        removeLabel={(index) => t('inspector.discount.removeRow', 'Remove code {n}', { n: index + 1 })}
        removeClassName="h-8"
        onAdd={() => edits.addRow('discCodes', { code: 'NEWCODE', label: t('inspector.discount.seedLabel', 'New discount'), amount: '0' })}
        ...
        renderRow={(row, index, remove) => (
          <div key={index} data-testid="invoices-row" className="flex flex-col gap-[5px] rounded-[10px] border border-border bg-surface-2 p-[9px]">
            <div className="flex gap-[6px]">
              <input
                type="text"
                aria-label={t('inspector.discount.code', 'Code {n}', { n: index + 1 })}
                placeholder={t('inspector.discount.codePlaceholder', 'CODE')}
                value={row.code}
              ...
              <input
                type="text"
                inputMode="decimal"
                aria-label={t('inspector.discount.amount', 'Code {n} amount', { n: index + 1 })}
                placeholder="0"
                value={row.amount}
                onFocus={edits.beginEdit}
            ...
            <input
              type="text"
              aria-label={t('inspector.discount.label', 'Code {n} description', { n: index + 1 })}
              placeholder={t('inspector.discount.labelPlaceholder', 'Description')}
              value={row.label}
              onFocus={edits.beginEdit}
