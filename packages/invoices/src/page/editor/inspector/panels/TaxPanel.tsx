...
export function TaxPanel({ draft, edits, totals }: PanelProps) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <SuffixField label={t('inspector.tax.taxRate', 'Tax rate')} suffix="%" value={body.taxRate} onFocus={edits.beginEdit} onChange={(value) => edits.set('taxRate', value)} testId="invoices-tax-rate" />
      <SuffixField label={t('inspector.tax.discount', 'Discount')} suffix="%" value={body.discountRate} onFocus={edits.beginEdit} onChange={(value) => edits.set('discountRate', value)} testId="invoices-discount-rate" />
      <div data-testid="invoices-tax-ladder" className="flex flex-col gap-[9px] rounded-xl border border-border bg-surface-2 p-[14px]">
        <div className="flex justify-between text-[12.5px]">
          <span className="text-fg-muted">{t('inspector.tax.subtotal', 'Subtotal')}</span>
          <span className="font-mono font-bold text-fg tabular-nums">{money(totals.subtotal)}</span>
        </div>
        {parseDecimal(body.discountRate) > 0 ? (
          <div className="flex justify-between text-[12.5px]">
            <span className="text-fg-muted">{t('inspector.tax.discountRow', 'Discount')}</span>
        ...
        <div className="flex justify-between text-[12.5px]">
          <span className="text-fg-muted">{t('inspector.tax.tax', 'Tax')}</span>
        ...
        <div className="flex items-center justify-between border-t border-border pt-[9px]">
          <span className="text-[13px] font-extrabold text-fg">{t('inspector.tax.total', 'Total')}</span>
