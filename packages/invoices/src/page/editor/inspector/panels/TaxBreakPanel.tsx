...
export function TaxBreakPanel({ draft, edits, onSelect }: PanelProps) {
  return (
    <div className="flex flex-col gap-[9px]">
      <PanelLabel className="mb-0">{t('inspector.taxbreak.components', 'Tax components')}</PanelLabel>
      <RowsEditor
        rows={draft.body.taxLines}
        addLabel={t('inspector.taxbreak.add', 'Add tax line')}
        removeLabel={(index) => t('inspector.taxbreak.removeRow', 'Remove tax line {n}', { n: index + 1 })}
        onAdd={() => edits.addRow('taxLines', { label: t('inspector.taxbreak.seedLabel', 'New tax'), rate: '0' })}
        ...
        renderRow={(row, index, remove) => (
          <div key={index} data-testid="invoices-row" className="flex gap-[6px]">
            <input
              type="text"
              aria-label={t('inspector.taxbreak.label', 'Tax line {n} label', { n: index + 1 })}
              value={row.label}
            ...
            />
            <SuffixField compact suffix="%" ariaLabel={t('inspector.taxbreak.rate', 'Tax line {n} rate', { n: index + 1 })} value={row.rate} onFocus={edits.beginEdit} onChange={(rate) => edits.updateRow('taxLines', index, { rate })} />
            {remove}
          </div>
        )}
      />
      <Note>{t('inspector.taxbreak.note', 'Each rate is applied to the subtotal after any discount.')}</Note>
