      ...
      <div className="flex items-center gap-[10px]">
        <div className="min-w-0 flex-1">
          <div id="invoices-terms-checked-label" className="text-[12.5px] font-bold text-fg">
            {t('inspector.terms.preChecked', 'Pre-checked')}
          </div>
          <div className="text-[10.5px] text-fg-subtle">{t('inspector.terms.preCheckedHint', 'Show the box already ticked')}</div>
        </div>
        <Toggle on={body.termsChecked} labelledBy="invoices-terms-checked-label" onClick={() => edits.histSet('termsChecked', !body.termsChecked)} />
      </div>
      <TextAreaField label={t('inspector.terms.checkboxLabel', 'Checkbox label')} rows={3} value={body.termsLabel} onFocus={edits.beginEdit} onChange={(value) => edits.set('termsLabel', value)} testId="invoices-terms-label" />
