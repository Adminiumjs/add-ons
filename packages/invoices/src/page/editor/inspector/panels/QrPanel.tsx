...
export function QrPanel({ draft, edits, totals, onSelect }: PanelProps) {
  ...
  return (
    <div className="flex flex-col gap-[14px]">
      ...
      <TextField label={t('inspector.qr.caption', 'Caption')} value={body.qrCaption} onFocus={edits.beginEdit} onChange={(value) => edits.set('qrCaption', value)} className="text-[12.5px]" testId="invoices-qr-caption" />
      <Hint icon={Info} testId="invoices-qr-hint">
        {t('inspector.qr.hint', 'Encodes Amount due · {total}. The code shown is the image you upload under Images.', { total: formatMoney(totals.total, body.currency, body.cents) })}
      </Hint>
