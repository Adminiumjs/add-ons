...
export function CustomPanel({ section, edits, onSelect, onImageRejected }: PanelProps & { section: CustomSection }) {
  ...
  return (
    <div className="flex flex-col gap-[14px]">
      <TextField label={t('inspector.custom.title', 'Section title')} value={section.title} onFocus={edits.beginEdit} onChange={(title) => edits.updateCustom(id, { title })} className="font-bold" testId="invoices-custom-title" />
      {section.type === 'text' ? (
        <TextAreaField label={t('inspector.custom.body', 'Body copy')} rows={6} value={section.body} onFocus={edits.beginEdit} onChange={(body) => edits.updateCustom(id, { body })} className="py-[9px]" testId="invoices-custom-body" />
      ) : null}
      {section.type === 'image' ? (
        <>
          <div>
            <PanelLabel>{t('inspector.custom.image', 'Image')}</PanelLabel>
            <FileLabel
            ...
            >
              ...
              {t('inspector.custom.upload', 'Upload / replace')}
            </FileLabel>
          </div>
          <TextField label={t('inspector.custom.caption', 'Caption')} value={section.caption} onFocus={edits.beginEdit} onChange={(caption) => edits.updateCustom(id, { caption })} className="text-[12.5px]" testId="invoices-custom-caption" />
          <div className="flex items-center gap-[9px]">
            <span className="whitespace-nowrap text-[11px] text-fg-subtle" aria-hidden="true">
              {t('inspector.custom.height', 'Height')}
            </span>
            <RangeInput value={section.height} min={110} max={380} ariaLabel={t('inspector.custom.height', 'Height')} onBegin={edits.beginEdit} onChange={(height) => edits.updateCustom(id, { height })} testId="invoices-custom-height" />
          </div>
        </>
      ) : null}
      {section.type === 'kv' ? (
        <DashedButton onClick={() => edits.addCustomRow(id)} testId="invoices-rows-add" className="rounded-[10px] p-[9px] text-[12px]">
          ...
          {t('inspector.custom.addRow', 'Add row')}
        </DashedButton>
      ) : null}
      {section.type === 'gallery' ? <div className="text-[11.5px] leading-[1.55] text-fg-muted">{t('inspector.custom.galleryHint', 'Click each slot on the invoice to upload an image.')}</div> : null}
      <button
        type="button"
      ...
      >
        ...
        {t('inspector.custom.remove', 'Remove section')}
      </button>
    </div>
  );
}
