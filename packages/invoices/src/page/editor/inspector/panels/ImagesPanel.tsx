...
function slotText(field: ImageField): { label: string; hint: string; icon: string } {
  switch (field) {
    case 'logoImage':
      return { label: t('inspector.images.logo', 'Logo'), hint: t('inspector.images.logoHint', 'Replaces the logo mark'), icon: 'hexagon' };
    case 'bgImage':
      return { label: t('inspector.images.background', 'Background'), hint: t('inspector.images.backgroundHint', 'Watermark behind the invoice'), icon: 'image' };
    case 'qrImage':
      return { label: t('inspector.images.qr', 'QR code'), hint: t('inspector.images.qrHint', 'Shown in the payment QR block'), icon: 'qr-code' };
    case 'sigImage':
      return { label: t('inspector.images.signature', 'Signature'), hint: t('inspector.images.signatureHint', 'Scanned signature image'), icon: 'pen-line' };
    case 'stampImage':
      return { label: t('inspector.images.stamp', 'Stamp / seal'), hint: t('inspector.images.stampHint', 'Paid or approval stamp'), icon: 'stamp' };
  }
}
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="text-[11.5px] leading-[1.55] text-fg-muted">
        {t('inspector.images.intro', 'Fixed images that travel with the invoice. Upload once and every document built from this template keeps them.')}
      </div>
        ...
        return (
          <div key={field} data-testid="invoices-image-slot" data-field={field} data-filled={url !== ''} className="flex items-center gap-[11px] rounded-xl border border-border bg-surface-2 p-[11px]">
            {url === '' ? (
              <FileLabel
                onFile={upload}
                ariaLabel={t('inspector.images.uploadSlot', 'Upload {label}', { label })}
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex flex-wrap gap-[6px]">
                <FileLabel onFile={upload} className={`${SMALL_BUTTON} text-fg-muted hover:border-border-strong`}>
                  {url === '' ? t('inspector.images.upload', 'Upload') : t('inspector.images.replace', 'Replace')}
                </FileLabel>
                {url === '' ? null : (
                  <button type="button" data-testid="invoices-image-remove" onClick={() => edits.setImage(field, '')} className={`${SMALL_BUTTON} text-danger transition-colors hover:border-border-strong`}>
                    {t('inspector.images.remove', 'Remove')}
                  </button>
                )}
              </div>
            </div>
      ...
      <DashedButton onClick={onOpenAdd} testId="invoices-images-add-section" className="gap-[7px] rounded-[10px] p-[10px] text-[12px]">
        ...
        {t('inspector.images.addSection', 'Add an image section')}
      </DashedButton>
    </div>
  );
}
