...
export function AttachmentsPanel({ draft, edits, onSelect }: PanelProps) {
  return (
    <div className="flex flex-col gap-[9px]">
      <PanelLabel className="mb-0">{t('inspector.attachments.files', 'Files')}</PanelLabel>
      <RowsEditor
        rows={draft.body.attachments}
        addLabel={t('inspector.attachments.add', 'Add file')}
        removeLabel={(index) => t('inspector.attachments.removeRow', 'Remove file {n}', { n: index + 1 })}
        onAdd={() => edits.addRow('attachments', { name: t('inspector.attachments.seedName', 'New file.pdf'), size: '—' })}
        ...
        renderRow={(file, index, remove) => (
          <div key={index} data-testid="invoices-row" className="flex gap-[6px]">
            <input
              type="text"
              aria-label={t('inspector.attachments.name', 'File {n} name', { n: index + 1 })}
              value={file.name}
            ...
            <input
              type="text"
              aria-label={t('inspector.attachments.size', 'File {n} size', { n: index + 1 })}
              value={file.size}
              onFocus={edits.beginEdit}
