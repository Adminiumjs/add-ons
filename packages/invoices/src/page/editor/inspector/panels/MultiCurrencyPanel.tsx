...
export function MultiCurrencyPanel({ draft, edits, onSelect }: PanelProps) {
  return (
    <div className="flex flex-col gap-[9px]">
      <PanelLabel className="mb-0">{t('inspector.multicurrency.rates', 'Currencies & rates')}</PanelLabel>
      <RowsEditor
        rows={draft.body.fx}
        addLabel={t('inspector.multicurrency.add', 'Add currency')}
        removeLabel={(index) => t('inspector.multicurrency.removeRow', 'Remove currency {n}', { n: index + 1 })}
        removeClassName="h-8"
        ...
        renderRow={(row, index, remove) => (
          <div key={index} data-testid="invoices-row" className="flex gap-[6px]">
            <input
              type="text"
              aria-label={t('inspector.multicurrency.code', 'Currency {n} code', { n: index + 1 })}
              value={row.code}
            ...
            <input
              type="text"
              aria-label={t('inspector.multicurrency.symbol', 'Currency {n} symbol', { n: index + 1 })}
              value={row.sym}
              onFocus={edits.beginEdit}
            ...
            <input
              type="text"
              inputMode="decimal"
              aria-label={t('inspector.multicurrency.rate', 'Currency {n} rate', { n: index + 1 })}
              value={row.rate}
              onFocus={edits.beginEdit}
      ...
      />
      <Note>{t('inspector.multicurrency.note', 'Rate is multiplied by the invoice total. Code, symbol, then rate.')}</Note>
