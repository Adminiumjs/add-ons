...
export function stepStatusLabel(status: DeliveryStepStatus): string {
  switch (status) {
    case 'todo':
      return t('inspector.delivery.status.todo', 'Pending');
    case 'current':
      return t('inspector.delivery.status.current', 'In progress');
    case 'done':
      return t('inspector.delivery.status.done', 'Done');
  }
}
...
export function DeliveryPanel({ draft, edits, onSelect }: PanelProps) {
  return (
    <div className="flex flex-col gap-[9px]">
      <PanelLabel className="mb-0">{t('inspector.delivery.steps', 'Steps')}</PanelLabel>
      <RowsEditor
        rows={draft.body.delSteps}
        addLabel={t('inspector.delivery.add', 'Add step')}
        removeLabel={(index) => t('inspector.delivery.removeRow', 'Remove step {n}', { n: index + 1 })}
        onAdd={() => edits.addRow('delSteps', { label: t('inspector.delivery.seedLabel', 'New step'), status: 'todo' })}
        ...
        renderRow={(row, index, remove) => (
          <div key={index} data-testid="invoices-row" className="flex items-center gap-[6px]">
            <input
              type="text"
              aria-label={t('inspector.delivery.label', 'Step {n}', { n: index + 1 })}
              value={row.label}
            ...
            <button
              ...
              aria-label={t('inspector.delivery.cycle', 'Step {n} status: {status}', { n: index + 1, status: stepStatusLabel(row.status) })}
      ...
      />
      <Note>{t('inspector.delivery.note', 'Tap the status to cycle Pending → In progress → Done.')}</Note>
