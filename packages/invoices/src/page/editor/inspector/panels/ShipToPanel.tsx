...
export function ShipToPanel({ draft, edits, onSelect }: PanelProps) {
  const lines = t('inspector.shipto.addressLines', 'Address lines');
  return (
    <div className="flex flex-col gap-[9px]">
      <TextField label={t('inspector.shipto.name', 'Ship-to name')} labelClassName="mb-[9px]" value={draft.body.shipName} onFocus={edits.beginEdit} onChange={(value) => edits.set('shipName', value)} className="font-bold" testId="invoices-ship-name" />
