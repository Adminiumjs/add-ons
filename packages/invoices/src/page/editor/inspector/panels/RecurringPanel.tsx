...
export function frequencyLabel(frequency: string): string {
  switch (frequency) {
    case 'Weekly':
      return t('inspector.recurring.weekly', 'Weekly');
    case 'Monthly':
      return t('inspector.recurring.monthly', 'Monthly');
    case 'Quarterly':
      return t('inspector.recurring.quarterly', 'Quarterly');
    case 'Annually':
      return t('inspector.recurring.annually', 'Annually');
    default:
      return frequency;
  }
}
    ...
    <div className="flex flex-col gap-[14px]">
      <div>
        <PanelLabel id="invoices-frequency-label" className="mb-2">
          {t('inspector.recurring.frequency', 'Frequency')}
        </PanelLabel>
      ...
      <TextField label={t('inspector.recurring.next', 'Next issue date')} value={body.recurNext} onFocus={edits.beginEdit} onChange={(value) => edits.set('recurNext', value)} className="text-[12.5px]" testId="invoices-recur-next" />
      <TextField label={t('inspector.recurring.note', 'Schedule note')} value={body.recurCount} onFocus={edits.beginEdit} onChange={(value) => edits.set('recurCount', value)} className="text-[12.5px]" testId="invoices-recur-count" />
