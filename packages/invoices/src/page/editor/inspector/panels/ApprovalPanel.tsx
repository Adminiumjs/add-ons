...
export function approvalLabel(status: ApprovalStatus): string {
  switch (status) {
    case 'pending':
      return t('inspector.approval.status.pending', 'Pending');
    case 'approved':
      return t('inspector.approval.status.approved', 'Approved');
    case 'rejected':
      return t('inspector.approval.status.rejected', 'Rejected');
  }
}
...
export function ApprovalPanel({ draft, edits, onSelect }: PanelProps) {
  ...
  return (
    <div className="flex flex-col gap-[14px]">
      <TextField label={t('inspector.approval.name', 'Approver name')} value={body.apprName} onFocus={edits.beginEdit} onChange={(value) => edits.set('apprName', value)} className="font-bold" testId="invoices-appr-name" />
      <TextField label={t('inspector.approval.title', 'Role / title')} value={body.apprTitle} onFocus={edits.beginEdit} onChange={(value) => edits.set('apprTitle', value)} className="text-[12.5px]" testId="invoices-appr-title" />
      <div>
        <PanelLabel id="invoices-approval-status-label" className="mb-2">
          {t('inspector.approval.statusLabel', 'Status')}
        </PanelLabel>
