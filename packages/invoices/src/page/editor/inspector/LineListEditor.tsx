  ...
  return (
    <>
      {lines.map((line, index) => {
        const name = t('inspector.line', '{label} {n}', { label, n: index + 1 });
        return (
          <div key={index} data-testid="invoices-line" className="flex gap-[6px]">
            ...
            />
            <RowRemoveButton label={t('inspector.removeLine', 'Remove {label} {n}', { label, n: index + 1 })} onClick={() => edits.removeLine(field, index)} className="w-[32px] rounded-[9px]" iconClassName="size-3.5" />
          </div>
        );
      })}
      <DashedButton onClick={() => edits.addLine(field)} testId="invoices-rows-add">
        <Plus className="size-3.5" aria-hidden="true" />
        {t('inspector.addLine', 'Add line')}
      </DashedButton>
    </>
  );
}
