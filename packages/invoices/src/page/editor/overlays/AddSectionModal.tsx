...
export function AddSectionModal({ body, onClose, onPickCustom, onPickBuiltin }: AddSectionModalProps) {
  ...
  return (
    ...
    >
      <ModalHeader
        icon={<LayoutList />}
        title={t('add.title', 'Add a section')}
        subtitle={t('add.subtitle', 'Build your own, or switch on one of the standard blocks.')}
        closeLabel={t('common.close', 'Close')}
      />
      <ModalBody data-testid="invoices-add-modal" className="flex flex-col gap-[18px]">
        <section>
          <div className={EYEBROW}>{t('add.custom', 'Build your own')}</div>
        ...
        <section>
          <div className={EYEBROW}>{t('add.standard', 'Standard blocks')}</div>
          {off.length === 0 ? (
            <div data-testid="invoices-add-none" className="text-[11.5px] leading-[1.5] text-fg-subtle">
              {t('add.allOn', 'Every standard block is already on this invoice.')}
            </div>
          ) : (
