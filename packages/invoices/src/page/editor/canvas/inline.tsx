...
export function SelectButton({ label, onSelect }: { label: string; onSelect: () => void }) {
  return (
    ...
    >
      {t('canvas.select', 'Edit {label}', { label })}
    </button>
  );
}
...
export function Region({ section, selected, onSelect, label, className, children }: RegionProps) {
  const name = label ?? (customIdOf(section) === null ? fixedSectionHeader(section as FixedSectionKey).title : t('section.custom.title', 'Custom section'));
...
export function lineLabel(kind: 'from' | 'customer' | 'ship' | 'payment', index: number): string {
  ...
  switch (kind) {
    case 'from':
      return t('canvas.lines.from', 'From line {n}', { n });
    case 'customer':
      return t('canvas.lines.customer', 'Customer line {n}', { n });
    case 'ship':
      return t('canvas.lines.ship', 'Shipping line {n}', { n });
    case 'payment':
      return t('canvas.lines.payment', 'Payment line {n}', { n });
  }
}
