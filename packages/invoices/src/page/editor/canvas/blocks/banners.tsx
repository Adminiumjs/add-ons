...
function approvalMeta(status: ApprovalStatus): { label: string; tile: string; badge: string; Icon: LucideIcon } {
  switch (status) {
    case 'approved':
      return { label: t('canvas.approval.approved', 'Approved'), tile: 'bg-pos-soft text-pos', badge: 'bg-pos-soft text-pos', Icon: BadgeCheck };
    case 'rejected':
      return { label: t('canvas.approval.rejected', 'Rejected'), tile: 'bg-danger-soft text-danger', badge: 'bg-danger-soft text-danger', Icon: CircleX };
    case 'pending':
      return { label: t('canvas.approval.pending', 'Pending'), tile: 'bg-warn-soft text-warn', badge: 'bg-warn-soft text-warn', Icon: Clock };
  }
}
...
export function ApprovalBlock({ body, section, onSelect }: BlockProps) {
  ...
  return (
    <Region section="approval" selected={section} onSelect={onSelect}>
      <div className={`${KICKER} mb-[9px]`}>{t('canvas.approval.title', 'Approval')}</div>
    ...
    <Region section="latefees" selected={section} onSelect={onSelect}>
      <div className="flex items-start gap-[11px] rounded-[11px] bg-warn-soft px-[15px] py-[13px]">
        ...
        <div className="min-w-0">
          <div className="text-[12.5px] font-extrabold text-warn">{t('canvas.latefees.title', 'Late payment fee')}</div>
          <div data-testid="invoices-latefees-sentence" className="mt-0.5 text-[12px] leading-[1.55] text-[#6b6b76]">
            {t('canvas.latefees.sentence', 'A late fee of {rate}% per month applies to balances unpaid more than {days} days past the due date.', {
  ...
  return (
    <Region section="recurring" selected={section} onSelect={onSelect}>
      <div className={cn('flex items-center gap-3 rounded-[11px] px-[15px] py-[13px]', ACCENT_SOFT_BG)}>
        ...
        <div className="min-w-0 flex-1">
          <div className="text-[12.5px] font-extrabold text-[#191920]">{t('canvas.recurring.title', 'Recurring — {freq}', { freq: body.recurFreq })}</div>
          <div className="mt-0.5 text-[11.5px] text-[#6b6b76]">
            {t('canvas.recurring.next', 'Next on {next} · {count}', { next: body.recurNext, count: body.recurCount })}
          </div>
        </div>
  ...
  return (
    <Region section="loyalty" selected={section} onSelect={onSelect}>
      <div className={cn('flex items-center gap-[13px] rounded-[11px] px-[15px] py-[13px]', ACCENT_SOFT_BG)}>
        ...
        <div className="min-w-0 flex-1">
          <div className={cn('text-[10.5px] font-bold uppercase tracking-[.05em]', ACCENT_TEXT)}>{t('canvas.loyalty.title', 'Loyalty balance')}</div>
          <div data-testid="invoices-loyalty-balance" className="font-mono text-[17px] font-extrabold text-[#191920]">
            {t('canvas.loyalty.balance', '{balance} pts · {level}', { balance: body.loyBalance.toLocaleString('en-US'), level: body.loyLevel })}
          </div>
        </div>
