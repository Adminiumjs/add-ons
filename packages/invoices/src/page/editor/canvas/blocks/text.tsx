  ...
  return (
    <div className="grid grid-cols-2 gap-[18px] border-t border-[#ececef] pt-[22px]">
      <Region section="payment" selected={section} onSelect={onSelect}>
        <div className={`${KICKER} mb-[7px]`}>{t('canvas.payment', 'Payment')}</div>
      ...
      <Region section="notes" selected={section} onSelect={onSelect}>
        <div className={`${KICKER} mb-[7px]`}>{t('canvas.notes', 'Notes')}</div>
        <InlineTextarea label={t('canvas.notes', 'Notes')} value={body.notes} onFocus={edits.beginEdit} onChange={(value) => edits.set('notes', value)} className={PROSE} />
      </Region>
    </div>
  );
}
...
export function SignatureBlock({ body, edits, section, onSelect }: BlockProps) {
  return (
    <Region section="signature" selected={section} onSelect={onSelect}>
      <div className={`${KICKER} mb-3.5`}>{t('canvas.signature', 'Signature')}</div>
      <div className="grid grid-cols-2 gap-6">
        <div>
          ...
          <InlineInput label={t('canvas.sigName', 'Signer name')} value={body.sigName} onFocus={edits.beginEdit} onChange={(value) => edits.set('sigName', value)} className="text-[13px] font-bold" />
          <InlineInput label={t('canvas.sigTitle', 'Signer title')} value={body.sigTitle} onFocus={edits.beginEdit} onChange={(value) => edits.set('sigTitle', value)} className="mt-0.5 text-[11.5px] text-[#6b6b76]" />
        </div>
        <div>
          <div className="mb-[9px] h-[38px] border-b-[1.5px] border-[#e2e2e8]" />
          <div className="text-[11.5px] text-[#6b6b76]">{t('canvas.dateSigned', 'Date signed')}</div>
        </div>
      </div>
    </Region>
  );
    ...
    <Region section="terms" selected={section} onSelect={onSelect}>
      <div className="flex items-start gap-[11px]">
        <button
          ...
          aria-label={body.termsLabel.trim() === '' ? t('canvas.termsAccepted', 'Terms accepted') : body.termsLabel}
        ...
        <InlineInput
          label={t('canvas.termsLabel', 'Terms label')}
...
export function PotermsBlock({ body, edits, section, onSelect }: BlockProps) {
  return (
    <Region section="poterms" selected={section} onSelect={onSelect}>
      <div className={`${KICKER} mb-[7px]`}>{t('canvas.poTerms', 'Purchase order terms')}</div>
      <InlineTextarea label={t('canvas.poTerms', 'Purchase order terms')} value={body.poTerms} onFocus={edits.beginEdit} onChange={(value) => edits.set('poTerms', value)} className={PROSE} />
    </Region>
  );
}
    ...
    <Region section="legal" selected={section} onSelect={onSelect}>
      <div className="border-t border-[#ececef] pt-4">
        <div className="mb-1.5 flex items-center gap-[7px]">
          ...
          <span className={KICKER}>{t('canvas.legal', 'Legal')}</span>
        </div>
        <InlineTextarea
          label={t('canvas.legal', 'Legal')}
...
export function RefundBlock({ body, edits, section, onSelect }: BlockProps) {
  return (
    <Region section="refund" selected={section} onSelect={onSelect}>
      <div className={`${KICKER} mb-[7px]`}>{t('canvas.refund', 'Refund policy')}</div>
      <InlineTextarea label={t('canvas.refund', 'Refund policy')} value={body.refText} onFocus={edits.beginEdit} onChange={(value) => edits.set('refText', value)} className={PROSE} />
    </Region>
  );
}
...
export function ContactBlock({ body, section, onSelect }: BlockProps) {
  return (
    <Region section="contact" selected={section} onSelect={onSelect}>
      <div className={`${KICKER} mb-[9px]`}>{t('canvas.contact', 'Questions? Contact us')}</div>
