  ...
  return (
    <div className="grid grid-cols-2 gap-[18px]">
      <Region section="from" selected={section} onSelect={onSelect}>
        <div className={`${KICKER} mb-[7px]`}>{t('canvas.from', 'From')}</div>
      ...
      <Region section="customer" selected={section} onSelect={onSelect}>
        <div className={`${KICKER} mb-[7px]`}>{t('canvas.invoiceTo', 'Invoice to')}</div>
        <InlineInput
          label={t('canvas.customerName', 'Customer name')}
...
export function ShippingBlock({ body, edits, section, onSelect }: BlockProps) {
  return (
    <Region section="shipto" selected={section} onSelect={onSelect}>
      <div className={`${KICKER} mb-[7px]`}>{t('canvas.shipTo', 'Ship to')}</div>
      <InlineInput
        label={t('canvas.shipName', 'Shipping name')}
        value={body.shipName}
        onFocus={edits.beginEdit}
  ...
  return (
    <Region section="meta" selected={section} onSelect={onSelect}>
      <div className="grid grid-cols-4 gap-3.5">
        <div>
          <div className={`${KICKER} mb-[5px]`}>{t('canvas.issued', 'Issued')}</div>
          <InlineInput label={t('canvas.issued', 'Issued')} value={body.issued} onFocus={edits.beginEdit} onChange={(value) => edits.set('issued', value)} className={cell} mono />
        </div>
        <div>
          <div className={`${KICKER} mb-[5px]`}>{t('canvas.due', 'Due')}</div>
          <InlineInput label={t('canvas.due', 'Due')} value={body.due} onFocus={edits.beginEdit} onChange={(value) => edits.set('due', value)} className={cell} mono />
        </div>
        <div>
          <div className={`${KICKER} mb-[5px]`}>{t('canvas.terms', 'Terms')}</div>
          <InlineInput label={t('canvas.terms', 'Terms')} value={body.terms} onFocus={edits.beginEdit} onChange={(value) => edits.set('terms', value)} className={cell} />
        </div>
        <div>
          <div className={`${KICKER} mb-[5px]`}>{t('canvas.poNumber', 'PO number')}</div>
          <InlineInput label={t('canvas.poNumber', 'PO number')} value={body.poNumber} onFocus={edits.beginEdit} onChange={(value) => edits.set('poNumber', value)} className={cell} mono />
        </div>
      </div>
    </Region>
  );
