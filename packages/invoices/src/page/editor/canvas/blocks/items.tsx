  ...
  return (
    <Region section="items" selected={section} onSelect={onSelect}>
      <div className={cn(ROW_GRID, KICKER, 'pb-2')}>
        <span />
        <span>{t('canvas.items.description', 'Description')}</span>
        <span className="text-center">{t('canvas.items.qty', 'Qty')}</span>
        <span className="text-end">{t('canvas.items.rate', 'Rate')}</span>
        <span className="text-end">{t('canvas.items.amount', 'Amount')}</span>
        <span />
      </div>
      <div className="flex flex-col">
        {body.items.map((item, index) => {
          ...
          const rowName = item.desc.trim() === '' ? t('canvas.items.row', 'Line item {n}', { n: index + 1 }) : item.desc;
          return (
            <div
              key={item.id}
            ...
            >
              <button
                type="button"
                draggable
                title={t('canvas.items.reorder', 'Drag to reorder')}
                aria-label={t('canvas.items.reorderOf', 'Drag to reorder: {name}', { name: rowName })}
              ...
              <InlineInput
                label={t('canvas.items.descriptionOf', 'Description of line {n}', { n: index + 1 })}
                value={item.desc}
              ...
              />
              <InlineInput
                label={t('canvas.items.qtyOf', 'Quantity of line {n}', { n: index + 1 })}
                value={item.qty}
                onFocus={edits.beginEdit}
              ...
              />
              <InlineInput
                label={t('canvas.items.rateOf', 'Rate of line {n}', { n: index + 1 })}
                value={item.rate}
                onFocus={edits.beginEdit}
              ...
              <button
                type="button"
                aria-label={t('canvas.items.remove', 'Remove line item: {name}', { name: rowName })}
                onClick={(event) => {
      ...
      >
        ...
        {t('canvas.items.add', 'Add line item')}
      </button>
    </Region>
  );
}
      ...
      <Region section="tax" selected={section} onSelect={onSelect}>
        <div data-testid="invoices-totals" className="flex w-[280px] flex-col gap-[9px]">
          <div className="flex justify-between text-[13px]">
            <span className="text-[#6b6b76]">{t('canvas.totals.subtotal', 'Subtotal')}</span>
          ...
          {showDiscount ? (
            <div data-testid="invoices-discount-row" className="flex justify-between text-[13px]">
              <span className="text-[#6b6b76]">{t('canvas.totals.discount', 'Discount ({rate})', { rate: formatPercent(body.discountRate) })}</span>
              <span className="font-mono font-bold text-pos">{`−${money(totals.discount)}`}</span>
            </div>
          ) : null}
          <div className="flex justify-between text-[13px]">
            <span className="text-[#6b6b76]">{t('canvas.totals.tax', 'Tax ({rate})', { rate: formatPercent(body.taxRate) })}</span>
          ...
          <div className="flex items-center justify-between border-t border-[#ececef] pt-[11px]">
            <span className="text-[14px] font-extrabold">{t('canvas.totals.total', 'Total')}</span>
