...
export function CustomBlock({ custom, edits, section, onSelect, onRemoveCustom, onImageRejected }: CustomBlockProps) {
  ...
  const id = custom.id;
  const title = custom.title.trim() === '' ? t('section.custom.title', 'Custom section') : custom.title;
  return (
    <Region section={key} selected={section} onSelect={onSelect} label={title}>
      <div className="mb-2.5 flex items-center gap-2">
        <InlineInput
          label={t('canvas.custom.titleLabel', 'Section title')}
          value={custom.title}
        ...
        <button
          type="button"
          title={t('canvas.custom.remove', 'Remove section')}
          aria-label={t('canvas.custom.removeOf', 'Remove section: {title}', { title })}
      ...
      {custom.type === 'text' ? (
        <InlineTextarea
          label={t('canvas.custom.body', 'Section body')}
          value={custom.body}
          onFocus={edits.beginEdit}
        ...
        <>
          {custom.url === '' ? (
            ...
            >
              ...
              {t('canvas.custom.upload', 'Click to upload an image')}
              <input
                type="file"
                accept="image/*"
                className="sr-only"
            ...
            <div className="relative">
              ...
              <button
                type="button"
                title={t('canvas.custom.clearImage', 'Remove image')}
                aria-label={t('canvas.custom.clearImage', 'Remove image')}
                onClick={(event) => {
                  event.stopPropagation();
          ...
          <InlineInput
            label={t('canvas.custom.caption', 'Caption')}
              ...
              {image.url === '' ? (
                <label onClick={(event) => event.stopPropagation()} className={cn(DROP_LABEL, 'h-28 rounded-[10px]')}>
                  ...
                  <input
                    ...
                    accept="image/*"
                    className="sr-only"
                    aria-label={t('canvas.custom.uploadSlot', 'Upload image {n}', { n: index + 1 })}
                ...
                <>
                  ...
                  <button
                    type="button"
                    title={t('canvas.custom.clearSlot', 'Remove')}
                    aria-label={t('canvas.custom.clearSlotOf', 'Remove image {n}', { n: index + 1 })}
                    onClick={(event) => {
                      event.stopPropagation();
      ...
      {custom.type === 'kv' ? (
        <>
          <div className="overflow-hidden rounded-[11px] border border-[#ececef]">
            {custom.rows.map((row, index) => (
              <div key={index} data-testid="invoices-kv-row" className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_28px] items-center gap-2.5 border-b border-[#ececef] px-[13px] py-2 last:border-b-0">
                <InlineInput
                  label={t('canvas.custom.rowLabel', 'Label of row {n}', { n: index + 1 })}
                  value={row.k}
                  onFocus={edits.beginEdit}
                ...
                />
                <InlineInput
                  label={t('canvas.custom.rowValue', 'Value of row {n}', { n: index + 1 })}
                  value={row.v}
                  onFocus={edits.beginEdit}
                ...
                <button
                  type="button"
                  title={t('canvas.custom.removeRow', 'Remove row')}
                  aria-label={t('canvas.custom.removeRowOf', 'Remove row {n}', { n: index + 1 })}
                  onClick={(event) => {
                    event.stopPropagation();
          ...
          >
            ...
            {t('canvas.custom.addRow', 'Add row')}
          </button>
        </>
      ) : null}
    </Region>
