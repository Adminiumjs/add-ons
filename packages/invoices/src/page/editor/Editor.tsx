...
export function Editor({ detail }: EditorProps) {
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      failToast(t('editor.saveFailed', 'Could not save'), error);
    }
  ...
  useShortcut({
    id: 'invoices-save',
    group: 'Editing',
    label: t('editor.shortcutSave', 'Save the document'),
    keys: ['⌘', 'S'],
    handler: () => {
      void save();
    },
  ...
  const duplicate = useMutation({
    ...
    onSuccess: (copy) => {
      ...
      toasts.push({
        variant: 'success',
        title: kind === 'template' ? t('toast.duplicated.template', 'Template duplicated') : t('toast.duplicated.invoice', 'Invoice duplicated'),
        action: {
  ...
  const addLanguage = async (lang: InvoiceLang) => {
    ...
    try {
      ...
      else failToast(t('toast.languageFailed', 'Could not add that language'), error);
    } finally {
      setAddingLang(null);
    }
  };
      ...
      toasts.push({
        ...
        title:
          result.reason === 'tooLarge'
            ? t('toast.imageTooLarge', 'Image too large (max {max})', { max: IMAGE_MAX_LABEL })
            : result.reason === 'notImage'
              ? t('toast.notAnImage', 'That file is not an image')
              : t('toast.imageUnreadable', 'Could not read that file'),
      });
    },
    [toasts],
  );
