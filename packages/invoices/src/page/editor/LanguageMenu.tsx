...
export function languageStateLabel(state: LanguageRowState): string {
  switch (state) {
    case 'editing':
      return t('languages.state.editing', 'Editing');
    case 'open':
      return t('languages.state.open', 'Open');
    case 'create':
      return t('languages.state.create', 'Create');
  }
}
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          title={t('languages.title', 'Language variations')}
        >
          {current.toUpperCase()}
        </Button>
      </PopoverTrigger>
      <PopoverContent align="end" className="w-[272px] max-w-[78vw] rounded-[14px] p-[7px]" data-testid="invoices-language-menu">
        <div className="px-2.5 pb-1.5 pt-2 text-[10px] font-bold uppercase tracking-[.06em] text-fg-subtle">
          {t('languages.title', 'Language variations')}
        </div>
        ...
        <div className="px-2.5 pb-1 pt-2 text-[10.5px] leading-[1.5] text-fg-subtle">
          {t('languages.footnote', 'Creating a language makes a linked copy, grouped under the same topic.')}
        </div>
      </PopoverContent>
    </Popover>
  );
