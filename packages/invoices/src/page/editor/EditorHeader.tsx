    ...
    >
      <div className="flex min-w-[120px] flex-[0_1_auto] items-center gap-[9px] overflow-hidden">
        <Tag tone="neutral" className="shrink-0 gap-[5px] rounded-[20px] px-[9px] py-[3px] text-[10.5px]">
          ...
          {isTemplate ? t('editor.kind.template', 'Template') : t('editor.kind.invoice', 'Invoice')}
        </Tag>
        <input
          data-testid="invoices-editor-name"
          aria-label={t('editor.nameLabel', 'Name')}
          value={name}
          maxLength={120}
          onFocus={onNameFocus}
      ...
      <div className="ms-auto flex flex-wrap items-center justify-end gap-2">
        <div className="flex gap-0.5 rounded-[9px] border border-border bg-surface-2 p-[3px]">
          <IconButton variant="ghost" size="md" label={t('editor.undo', 'Undo')} onClick={onUndo} disabled={!canUndo} data-testid="invoices-undo">
            <Undo2 className="size-[15px]" />
          </IconButton>
          <IconButton variant="ghost" size="md" label={t('editor.redo', 'Redo')} onClick={onRedo} disabled={!canRedo} data-testid="invoices-redo">
        ...
        <IconButton variant="bordered" size="xl" label={t('editor.images', 'Images')} tooltip onClick={onImages} data-testid="invoices-images">
          <Image className="size-4" />
        </IconButton>
        <IconButton variant="bordered" size="xl" label={t('editor.duplicate', 'Duplicate')} tooltip onClick={onDuplicate} data-testid="invoices-duplicate">
          <Copy className="size-4" />
        </IconButton>
        <IconButton variant="bordered" size="xl" label={t('editor.delete', 'Delete')} tooltip onClick={onDelete} data-testid="invoices-delete">
