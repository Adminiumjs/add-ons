...
export function ListHeader() {
  return (
    <div role="row" className={cn(GRID, 'border-b border-border bg-surface-2 py-[11px] text-[10.5px] font-bold uppercase tracking-[.05em] text-fg-subtle')}>
      <span role="columnheader">{t('list.name', 'Name')}</span>
      <span role="columnheader">{t('list.status', 'Status')}</span>
      <span role="columnheader">{t('list.updated', 'Updated')}</span>
  ...
  return (
    ...
    >
      ...
      <div role="cell" data-testid="invoices-row-actions" className="flex justify-end gap-[5px]">
        <RowAction label={t('card.duplicate', 'Duplicate')} onClick={() => actions.onDuplicate(doc)}>
          <Copy className="size-[13px]" aria-hidden="true" />
        </RowAction>
        <RowAction label={t('card.rename', 'Rename')} onClick={() => actions.onRename(doc)}>
          <TextCursorInput className="size-[13px]" aria-hidden="true" />
        </RowAction>
        <RowAction label={t('card.delete', 'Delete')} onClick={() => actions.onDelete(doc)}>
