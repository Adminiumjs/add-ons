...
export function topicLabel(topic: InvoiceTopic): string {
  switch (topic) {
    case 'recurring':
      return t('topic.recurring', 'Recurring');
    case 'services':
      return t('topic.services', 'Professional services');
    case 'receipts':
      return t('topic.receipts', 'Receipts & refunds');
    case 'sales':
      return t('topic.sales', 'Sales & quotes');
    case 'logistics':
      return t('topic.logistics', 'Shipping & logistics');
    case 'other':
      return t('topic.other', 'Uncategorised');
  }
}
...
export function languageLabel(code: InvoiceLang): string {
  switch (code) {
    case 'en':
      return t('languages.en', 'English');
    case 'de':
      return t('languages.de', 'German');
    case 'fr':
      return t('languages.fr', 'French');
    case 'es':
      return t('languages.es', 'Spanish');
    case 'pt':
      return t('languages.pt', 'Portuguese');
    case 'ja':
      return t('languages.ja', 'Japanese');
  }
}
...
export function categoryLabel(category: string): string {
  switch (category) {
    case 'business':
      return t('new.category.business', 'Business');
    case 'payments':
      return t('new.category.payments', 'Payments');
    case 'adjustments':
      return t('new.category.adjustments', 'Adjustments');
    case 'sales':
      return t('new.category.sales', 'Sales');
    case 'recurring':
      return t('new.category.recurring', 'Recurring');
    case 'services':
      return t('new.category.services', 'Services');
    case 'projects':
      return t('new.category.projects', 'Projects');
    case 'shipping':
      return t('new.category.shipping', 'Shipping');
    case 'nonprofit':
      return t('new.category.nonprofit', 'Nonprofit');
    default:
      return category;
  }
}
...
function documentsSub(count: number): string {
  return t('manager.group.documents', '{count, plural, one {# document} other {# documents}}', { count });
}

function languagesSub(count: number): string {
  return t('manager.group.languages', '{count, plural, one {# language} other {# languages}}', { count });
}
