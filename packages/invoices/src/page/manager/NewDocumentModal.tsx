...
import { useAppToasts } from '@adminium/add-on-contracts/runtime/app';
...
export function NewDocumentModal({ kind, onClose, onCreated }: NewDocumentModalProps) {
  ...
  const fail = (error: unknown) => {
    ...
    toasts.push({
      variant: 'error',
      title: t('new.failed', 'Could not create it'),
  ...
  const title = kind === 'template' ? t('new.template', 'New template') : t('new.invoice', 'New invoice');
  ...
  return (
    ...
    >
      <ModalHeader
        icon={<FilePlus2 />}
        title={title}
        subtitle={t('new.subtitle', 'Start from a blank canvas or a ready-made template.')}
      ...
      <ModalBody className="pt-1">
        {starters.isPending ? (
          ...
          <p role="alert" className="py-10 text-center text-body-sm text-danger">
            {t('new.startersFailed', 'The starters could not be loaded. Start blank, or try again.')}
          </p>
        ) : null}
        <div data-testid="invoices-new-grid" className="grid grid-cols-2 gap-3.5 sm:grid-cols-3 lg:grid-cols-4">
          ...
          >
            ...
            <div className="text-[13px] font-extrabold text-fg">{t('new.blank', 'Blank invoice')}</div>
            <div className="text-center text-[11px] leading-[1.4] text-fg-subtle">{t('new.blankHint', 'Build from scratch')}</div>
          </button>
        ...
        {kind === 'invoice' && yourTemplates.length > 0 ? (
          <section data-testid="invoices-your-templates" className="mt-6">
            <h3 className="mb-3 flex items-center gap-2 text-[10.5px] font-bold uppercase tracking-[.05em] text-fg-subtle">
              ...
              {t('new.yourTemplates', 'Your templates')}
            </h3>
