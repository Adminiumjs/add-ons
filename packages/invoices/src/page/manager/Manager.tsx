...
function emptyCopy(tab: InvoiceDocumentKind, searching: boolean): { title: string; body: string } {
  if (searching) {
    return {
      title: tab === 'template' ? t('empty.noMatch.templates', 'No templates match') : t('empty.noMatch.invoices', 'No invoices match'),
      body: t('empty.noMatch.body', 'Try a different search term.'),
    };
  }
  return tab === 'template'
    ? {
        title: t('empty.templates.title', 'No templates yet'),
        body: t('empty.templates.body', 'Create a reusable invoice template your team can build from.'),
      }
    : {
        title: t('empty.invoices.title', 'No invoices yet'),
        body: t('empty.invoices.body', 'Build your first invoice from a template or a blank canvas.'),
      };
}

/** `New {template|invoice}` (comp 188, 224; `tabSingular` 1466). */
export function newLabel(tab: InvoiceDocumentKind): string {
  return tab === 'template' ? t('new.template', 'New template') : t('new.invoice', 'New invoice');
}
...
export function InvoiceManager({ initialTab = 'template' }: InvoiceManagerProps) {
  ...
  const duplicate = useMutation({
    ...
    onSuccess: (copy: InvoiceDetail, doc) => {
      ...
      toasts.push({
        variant: 'success',
        title: doc.kind === 'template' ? t('toast.duplicated.template', 'Template duplicated') : t('toast.duplicated.invoice', 'Invoice duplicated'),
        action: {
    ...
    onError: (error) => failToast(t('toast.duplicateFailed', 'Could not duplicate it'), error),
  });
  ...
  const deleteDoc = (doc: InvoiceSummary) => {
    remove.mutate(doc.id, {
      ...
      onError: (error) => {
        setConfirm(null);
        failToast(t('toast.deleteFailed', 'Could not delete it'), error);
      },
    });
  };
  ...
  const commitRename = () => {
    ...
    const value = trimmed === '' ? t('manager.untitled', 'Untitled') : trimmed;
    const { id, original } = rename;
    setRenameState(null);
    if (value === original) return;
    patch.mutate({ id, name: value }, { onError: (error) => failToast(t('toast.renameFailed', 'Could not rename it'), error) });
  };
  ...
  const tabLabel = tab === 'template' ? t('manager.tabs.templates', 'Templates') : t('manager.tabs.invoices', 'Invoices');

  return (
    <>
      <PageActions title={t('manager.title', 'Invoices')} subtitle={t('manager.subtitle', 'Reusable templates & the invoices you build from them.')}>
          ...
          <TabsContent value={tab}>
            {list.isPending ? (
              ...
              <Alert
                role="alert"
                tone="danger"
                title={t('manager.loadFailed', 'Couldn’t load invoices')}
