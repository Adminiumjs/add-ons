...
export function DeleteModal({ kind, name, busy, onCancel, onConfirm }: DeleteModalProps) {
  return (
    ...
    >
      <ModalHeader icon={<Trash2 />} tone="danger" title={t('delete.title', 'Delete {name}?', { name })} closeLabel={t('common.close', 'Close')} />
      <ModalBody>
        <p data-testid="invoices-delete-body" className="text-body-sm text-fg-muted">
          {kind === 'template'
            ? t('delete.body.template', 'This can’t be undone. The template will be permanently removed.')
            : t('delete.body.invoice', 'This can’t be undone. The invoice will be permanently removed.')}
        </p>
      </ModalBody>
      <ModalFooter>
