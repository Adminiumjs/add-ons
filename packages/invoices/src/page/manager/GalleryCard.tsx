...
export function RenameInput({ rename, className }: { rename: RenameState; className?: string | undefined }) {
  ...
  return (
    <input
      ...
      aria-label={t('card.renameLabel', 'New name')}
      value={rename.value}
    ...
    >
      ...
      <div className="px-[15px] pb-3.5 pt-[13px]">
        ...
        <div data-testid="invoices-card-actions" className={cn('flex gap-[5px]', ACTION_ROW_REVEAL)}>
          ...
          <SquareAction label={t('card.duplicate', 'Duplicate')} onClick={() => actions.onDuplicate(doc)}>
            <Copy className="size-[13px]" aria-hidden="true" />
          </SquareAction>
          <SquareAction label={t('card.rename', 'Rename')} onClick={() => actions.onRename(doc)}>
            <TextCursorInput className="size-[13px]" aria-hidden="true" />
          </SquareAction>
          <SquareAction label={t('card.delete', 'Delete')} onClick={() => actions.onDelete(doc)}>
