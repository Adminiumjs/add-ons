...
export function searchPlaceholder(tab: InvoiceDocumentKind): string {
  return tab === 'template' ? t('manager.search.templates', 'Search templates…') : t('manager.search.invoices', 'Search invoices…');
}
    ...
    <div data-testid="invoices-toolbar" className="mb-5 flex flex-wrap items-center gap-3">
      ...
      <div className="ms-auto flex items-center gap-2">
        ...
        <SegmentedControl
          ...
          options={[
            { value: 'none', label: t('manager.group.none', 'None') },
            { value: 'topic', label: t('manager.group.topic', 'Topic'), icon: <FolderTree aria-hidden="true" /> },
            { value: 'language', label: t('manager.group.language', 'Language'), icon: <Languages aria-hidden="true" /> },
          ]}
        />
      </div>
      <SearchInput
        ...
        clearLabel={t('manager.search.clear', 'Clear search')}
      />
      <SegmentedControl
        aria-label={t('manager.layout.label', 'Layout')}
        data-testid="invoices-layout"
        value={layout}
        onValueChange={(value) => onLayoutChange(value === 'list' ? 'list' : 'gallery')}
        options={[
          { value: 'gallery', ariaLabel: t('manager.layout.gallery', 'Gallery'), icon: <LayoutGrid aria-hidden="true" /> },
          { value: 'list', ariaLabel: t('manager.layout.list', 'List'), icon: <List aria-hidden="true" /> },
        ]}
      />
    </div>
  );
