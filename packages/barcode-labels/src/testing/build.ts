 ...
 * therefore produces a DIFFERENT bundle from the one that ships, complete with
 ...
 *
 * `<root>/node_modules/.bin/vite` is true in a standalone clone and false in a
 * workspace: npm hoists a shared dependency to the monorepo root, the
 * per-package `.bin` link is not written, and the suite dies with a bare ENOENT
 * that reads like a broken install. `require.resolve` asks Node where the
 * module actually is, which is the same answer in a hoisted tree, a nested one
 * and a standalone clone.
 *
