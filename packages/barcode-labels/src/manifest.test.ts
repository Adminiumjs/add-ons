 ...
 *
 *   · that the manifest and `register()` fill the same two slots, and that both
 *     agree with `slots.ts`, which is narrowed against the host's own registry;
 *   · that the paths it declares are the file the build actually writes;
 ...
 *
 * ── AND ONE THING THAT IS THIS PACKAGE'S ALONE ─────────────────────────────
 *
 * IT NAMES `record.actions`, WHICH NOTHING HAD EVER NAMED. The slot was added
 * to the closed registry on 2026-08-28 and shipped with no fill; the registry's
 * own entry and `payloads.ts` both say so. The case below asserts this manifest
 * declares it, because that is the single claim the package is here to make,
 * and a manifest that quietly dropped it would leave the fill unreachable with
 * every other suite in the repository green.
 */
...
import { STORAGE_KEY } from './codes.ts';
 ...
 * same two slots. This set is the belt; that is the braces.
 */
...
describe('the manifest', () => {
  ...
  it('fills two slots from the closed registry, and register() fills the same two', () => {
    const slots = manifest.addOn.slots.map((fill) => fill.slot);
    expect([...slots].sort()).toEqual(['record.actions', 'settings.add-on.panel']);
    ...
    expect(
      register()
        ...
        .sort(),
    ).toEqual([...FILLED_SLOTS].sort());
  });

  /**
   * THE CLAIM THIS PACKAGE IS HERE TO MAKE.
   *
   * `record.actions` is the twelfth entry in the closed registry, bought on
   * 2026-08-28 against a seven-exhibit dossier and shipped UNFILLED — which is
   * the strongest criticism of the wave it shipped in. This manifest is the
   * first anywhere to name it. Asserted on its own, rather than left inside the
   * list above, because a manifest that quietly dropped it would leave the fill
   * unreachable with every other suite in the repository green.
   */
  it('names the slot nothing had ever named', () => {
    const fill = manifest.addOn.slots.find((entry) => entry.slot === 'record.actions');
    expect(fill, 'the manifest does not declare record.actions').toBeDefined();
    expect(fill!.client).toBe(OUTPUT.client);
    expect(register().fills.some((entry) => entry.slot === 'record.actions')).toBe(true);
  });
  ...
  it('asks for no scope, because it writes no host record', () => {
    // A scope is a request for power. This add-on is handed one record and
    // reads its identity; it stores its own table under its own settings key
    // and hands it over as data. The honest length of the list is zero, and it
    // also has no `requiredSchema` — it brings no table.
  ...
  it('declares the one key it writes, so an installer can see it', () => {
    ...
    const codes = manifest.settings[0]!;
    expect(codes.key).toBe(STORAGE_KEY);
    // `json`, because a table of rows and numbers is none of the controls a
    // HOST can draw — which is why `register().settings` is empty and the whole
    // form lives in `settings.add-on.panel`.
    expect(codes.type).toBe('json');
    expect(codes.default).toEqual([]);
  ...
  it('describes itself with the same sentence the bundle carries', () => {
    ...
    expect(manifest.description.key).toBe('addon.barcode-labels.line');
    expect(register().lineKey).toBe(manifest.description.key);
    expect(manifest.description.fallback).toBe(
      register().messages!['en-US']!['addon.barcode-labels.line'],
    );
  });
